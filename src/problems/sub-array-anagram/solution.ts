export function hasSubstringAnagram(s: string, anagram: string): boolean {
  ...
  const anagramSet = new Set(anagram.split(""));
  const sSet = new Set(s.slice(0,k).split(""));

  // If all the letter of the initial window are also part of the anagram,
  // then we have an anagram
  if (equalSets(anagramSet, sSet)) return true;
  ...
  for (let i = 0; i < s.length - k; i++) { // O(n)
    // n ops
    sSet.delete(s[i]);
    sSet.add(s[i + k]);
    if (equalSets(anagramSet,sSet)) return true; // O(k)
  }

  return false;
} // O(n*k)

// We check if sets are the same size and one is contained inside the other
